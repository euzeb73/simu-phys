...
class Link():
    def __init__(self,m1,m2):
        ...
        self.width=3 #largeur par défaut (affchage) mettre un entier impair SVP
        self.mass1=m1
        self.mass2=m2
        self.mass1.linklist.append((self,1)) #donne le lien et le numero de la masse pour ce lien
...
class LinkRigid(Link):
    ...
    def update(self):
        ...
        uortho=pygame.math.Vector2(-u[1],u[0]) #vecteur unitaire directement orthogonal
...
class LinkSpring(Link):
    ...
    def update(self):
        ...
        x2=self.mass2.OM
        l=norm(x1-x2)
        uM2M1=pygame.math.Vector2.normalize(x1-x2) #vecteur unitaire direction sens M2M1
    ...
    def draw(self,screen):
        ...
        if self.visible:
            ...
            for i in range(0,nseg):
                if i==0:
                    pointenm=self.mass1.OM+0.5*lseg*np.array([np.cos(alpha+theta),np.sin(theta+alpha)])
                elif i%2==0:
                    pointenm=pointenm+lseg*np.array([np.cos(alpha+theta),np.sin(alpha+theta)])
                else:
                    pointenm=pointenm+lseg*np.array([np.cos(theta-alpha),np.sin(theta-alpha)])
                point=OMtopx(pointenm)
                liste_points.append(point)
            arrivee=OMtopx(self.mass2.OM)
            liste_points.append(arrivee)
            pygame.draw.lines(screen,self.color,False,liste_points,self.width)
